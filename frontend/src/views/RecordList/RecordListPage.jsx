...
function RecordListPage() {
  ...
  const handlePrint = async () => {
    try {
      const res = await fetch(`/api/records?${buildQuery({ limit: 9999, offset: 0 })}`)
      const json = await res.json()
  ...
  const columns = useMemo(() => [
    {
      ...
      enableSorting: true,
      cell: (value) =>
        value ? dayjs(value).tz('Asia/Taipei').format('YYYY-MM-DD HH:mm:ss') : '',
    },
    ...
    { accessorKey: 'payment_method', header: '付款方式', cell: (v) => v ?? '' },
    {
      accessorKey: 'info',
      header: '資訊',
      cell: (v) => (v == null ? '' : typeof v === 'object' ? JSON.stringify(v) : String(v)),
    },
  ], [])
  ...
  return (
    <div className="record-list-container">
      ...
      <div className="record-header">
        ...
        <button className="print-button" onClick={handlePrint}>列印報表</button>
      </div>
