...
import "./CertificateRecordsPage.css";
...
function CertificateRecordsPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`/api/certificate-records?${buildQuery()}`);
      const json = await res.json();
      setData(Array.isArray(json.data) ? json.data : []);
      setTotal(json.total || 0);
    } catch (err) {
      console.error("Failed to fetch /api/certificate-records", err);
      setData([]);
    }
  };
  ...
  const handlePrint = async () => {
    try {
      const res = await fetch(
        `/api/certificate-records?${buildQuery({ limit: 9999, offset: 0 })}`,
      );
      const json = await res.json();
